...
| `editable` (`boolean`)                                  | `true`       | Set `false` for a read-only viewer: no typing, and the image toolkit, captions and table handles stay inert. Toggling it at runtime is supported.                                                                           |
...
| `onReady` (`(editor) => void`)                          | —            | Fires once when the bridge exists; the WebView may still be loading.                                                                                                                                                        |
| `onMounted` (`() => void`)                              | —            | The document has mounted in the WebView and can take focus — hide your own loading state here. Fires again after a renderer-crash remount.                                                                                  |
